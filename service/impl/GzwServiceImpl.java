...
public class GzwServiceImpl implements GzwService{
	...
	*/
	...
	public String add(Gzw model,LoginModel login){
		...
		gzwMapper.insertSelective(model);//插入数据
		return "";
	}
	/**
	修改
	*/
	...
	public String update(Gzw model,LoginModel login){
		Gzw preModel = gzwMapper.selectByPrimaryKey(model.getId());
		if(model.getLoginName()==null||model.getLoginName().equals("")){
			return "登录名为必填属性";
		}
		if(model.getLoginName()!=null){
		...
		gzwMapper.updateByPrimaryKey(preModel);//更新数据
		return "";
	}
	/**
	修改
	...
	*/
	...
	public Map<String,Integer> getDataListCount(Gzw queryModel,Integer pageSize,LoginModel login) {
		GzwExample se = new GzwExample();
		GzwExample.Criteria sc = se.createCriteria();
		...
		int count = (int)gzwMapper.countByExample(se);
		int totalPage = 0;
	...
	*/
	...
	public Map<String,Object> getDataList(Gzw queryModel,Integer page,Integer pageSize,LoginModel login) {
		...
		List<Gzw> list = gzwMapper.selectByExample(se);//执行查询语句
		Map<String,Object> rs = new HashMap<String,Object>();
		List<Map<String,Object>> list2 = new ArrayList<Map<String,Object>>();
		for(Gzw model:list){
			list2.add(getGzwModel(model,login));
		}
	...
	*/
	...
	public void delete(Integer id) {
		gzwMapper.deleteByPrimaryKey(id);
	}
}
