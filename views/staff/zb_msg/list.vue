	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="评标专家" style="min-width:250px;">
			...
			<div >
				<el-table
					...
					<el-table-column
						label="采购ID">
						<template v-slot="{ row }">
							<div v-html="''+(row.cgidStr==null?'':row.cgidStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="中标商">
						<template v-slot="{ row }">
							<div v-html="''+(row.zbMsg.zbs==null?'':row.zbMsg.zbs)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="联系人">
						<template v-slot="{ row }">
							<div v-html="''+(row.zbMsg.lxr==null?'':row.zbMsg.lxr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="联系方式">
						<template v-slot="{ row }">
							<div v-html="''+(row.zbMsg.lxfs==null?'':row.zbMsg.lxfs)+''"></div>
						</template>
					</el-table-column>
					<el-table-column label="中标文件"  width="200px">
						<template v-slot="{ row }">
							<template v-for="item2 in row.zbFileList">
					...
					<el-table-column
						label="评标专家">
						<template v-slot="{ row }">
							<div v-html="''+(row.pbzjStr==null?'':row.pbzjStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="中标时间">
						<template v-slot="{ row }">
							<div v-html="''+(row.zbMsg.zbTime==null?'':row.zbMsg.zbTime)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.zbMsg.id" @click="del1"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.zbMsg.id+''" data-href="/staff/zb_msg/update1_page" @click="goToPage"
				...
				<div>
					<div id="zb_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
...
export default {
	...
	mounted() {
		...
		that.cgid = (that.$route.query.cgid==undefined?'':that.$route.query.cgid);
		that.getInitData();
		$('#pbzj').next().remove();
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				...
				success : function(result) {
					that.pbzjList = admin_utils.addAllSelect(result.pbzjList);
				}
			});
		},
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/zb_msg/del1",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/zb_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"cgid":that.cgid,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/zb_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
