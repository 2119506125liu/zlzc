	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/staff/cg_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.cgMsg.id" @click="del1"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.cgMsg.id+''" data-href="/staff/cg_msg/update_page" @click="goToPage"
								type="warning" style="margin-left:5px;margin-top:5px;"
								>
								修改
							</el-button>
							<template v-if="row.cgMsg.cgStatus=='2'">
								<el-button
									href="javascript:void(0)" :data-id="row.cgMsg.id" @click="cgz"
									type="success" style="margin-left:5px;margin-top:5px;"
									>
									采购中
								</el-button>
							</template>
							<template v-if="row.cgMsg.cgStatus=='4'">
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.cgMsg.id+''" data-href="/staff/cg_msg/wccg_page" @click="goToPage"
									type="default" style="margin-left:5px;margin-top:5px;"
									>
									完成采购
								</el-button>
							</template>
							<template v-if="row.cgMsg.cgStatus=='4'">
								<el-button
									href="javascript:void(0)" :data-params="'cgid='+row.cgMsg.id+''" data-href="/staff/zb_msg/add1_page" @click="goToPage"
									type="warning" style="margin-left:5px;margin-top:5px;"
									>
									新增中标
								</el-button>
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/staff/cg_msg/getInitData",
				async: false,
				data : {
		...
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/cg_msg/del1",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		cgz(e){
			var that = this;
			var dataId  =e.currentTarget.dataset.id;
			var r=confirm("确认采购中该数据吗？");
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/cg_msg/cgz",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/cg_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"cpName":cpName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/cg_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
