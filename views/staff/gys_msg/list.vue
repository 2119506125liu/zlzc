	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="供应商名" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入供应商名" name="gysName" v-model="gysName" class="" id="gysName"></el-input>
				</el-form-item>
				<div style="display:inline-block">
					<el-form-item label="创建时间" style="min-width:200px;">
						<input type="date" id="createTime1"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
					<el-form-item style="min-width:200px;">
						<input type="date" id="createTime2"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
				</div>
				<el-form-item>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/staff/gys_msg/add1_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="供应商名">
						<template v-slot="{ row }">
							<div v-html="''+(row.gysMsg.gysName==null?'':row.gysMsg.gysName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="供应商电话">
						<template v-slot="{ row }">
							<div v-html="''+(row.gysMsg.gysCel==null?'':row.gysMsg.gysCel)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="供应商地址">
						<template v-slot="{ row }">
							<div v-html="''+(row.gysMsg.gysAddress==null?'':row.gysMsg.gysAddress)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="备注">
						<template v-slot="{ row }">
							<div v-html="''+(row.gysMsg.remark==null?'':row.gysMsg.remark)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="创建时间">
						<template v-slot="{ row }">
							<div v-html="''+(row.gysMsg.createTime==null?'':row.gysMsg.createTime)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.gysMsg.id" @click="del1"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.gysMsg.id+''" data-href="/staff/gys_msg/update1_page" @click="goToPage"
				...
				<div>
					<div id="gys_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/gys_msg/del1",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/gys_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"gysName":gysName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/gys_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
