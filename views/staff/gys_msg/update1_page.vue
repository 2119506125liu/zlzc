...
<template>
	<div class="edit-page-div">
		<div class="box-card">
			<h2 class="edit-page-div-h2">供应商信息-修改</h2>
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/staff/gys_msg/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.gysName=result.gysName;
					that.gysCel=result.gysCel;
					that.gysAddress=result.gysAddress;
		...
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/staff/gys_msg/update1_submit",
				async: false,
				data : {
					'id':that.id,
					'gysName':gysName,
