				...
				<el-table
					...
					<el-table-column label="封面图"  width="200px">
						...
						<el-table-column
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.notice.id+''" data-href="/staff/notice/detail" @click="goToPage"
...
export default {
	...
	methods:{
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/notice/queryCount',
				async: false,
				data : {
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/notice/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
