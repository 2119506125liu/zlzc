	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="专家名" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入专家名" name="zjName" v-model="zjName" class="" id="zjName"></el-input>
				</el-form-item>
				<el-form-item label="专家电话" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入专家电话" name="zjCel" v-model="zjCel" class="" id="zjCel"></el-input>
				</el-form-item>
				<el-form-item label="专家介绍" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入专家介绍" name="zjIntro" v-model="zjIntro" class="" id="zjIntro"></el-input>
				</el-form-item>
				<el-form-item>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/staff/pbzj/add1_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="专家名">
						<template v-slot="{ row }">
							<div v-html="''+(row.pbzj.zjName==null?'':row.pbzj.zjName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="专家电话">
						<template v-slot="{ row }">
							<div v-html="''+(row.pbzj.zjCel==null?'':row.pbzj.zjCel)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="专家介绍">
						<template v-slot="{ row }">
							<div v-html="''+(row.pbzj.zjIntro==null?'':row.pbzj.zjIntro)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.pbzj.id" @click="del1"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.pbzj.id+''" data-href="/staff/pbzj/update1_page" @click="goToPage"
				...
				<div>
					<div id="pbzj_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/pbzj/del1",
					async: false,
					data : {
		...
		ajaxCount(){
			var that = this;
			var rs = null;
			var zjName = that.zjName;
			var zjCel = that.zjCel;
			var zjIntro = that.zjIntro;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/pbzj/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"zjName":zjName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/pbzj/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
