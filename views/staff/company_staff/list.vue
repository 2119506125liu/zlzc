	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/staff/company_staff/add1_page" @click="goToPage"
					type="success" style="margin-left:5px;margin-top:5px;"
					>
					新增
				</el-button>
				<el-button
					href="javascript:void(0)" onclick="document.getElementById('import_company1_input').click()"
					type="warning" style="margin-left:5px;margin-top:5px;"
					>
					批量导入
				</el-button>
				<form id="import_company1_form"  style="margin:0px;display: inline" :action="''+REQUEST_URL+'/staff/company_staff/import_company1?loginToken='+currentLoginToken+'&'" enctype="multipart/form-data" method="post">
					<div style="display:none;"><input type="file" name="file" onclick="this.form.reset();" id="import_company1_input" data-id="import_company1_input" form-id="import_company1_form" class="import_input"/></div>
				</form>
				<el-button
				...
				<el-button
					href="javascript:void(0)" @click="export_company1"
					type="warning" style="margin-left:5px;margin-top:5px;"
					>
					批量导出
				</el-button>
			...
			<div >
				<el-table
					...
					<el-table-column
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.companyStaff.id+''" data-href="/staff/company_staff/update1_page" @click="goToPage"
								type="primary" style="margin-left:5px;margin-top:5px;"
								>
								修改
							</el-button>
							<el-button
								href="javascript:void(0)" :data-id="row.companyStaff.id" @click="del1"
	...
	methods:{
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/staff/company_staff/del1",
					async: false,
					data : {
		...
		exportModelExcel(){
			...
			window.location.href=this.REQUEST_URL+"/staff/company_staff/import_company1_model?"+params+"";
		},
		export_company1(e){
			var that = this;
			...
			params+="loginToken="+that.currentLoginToken+"&";
			window.location.href=this.REQUEST_URL+"/staff/company_staff/export_company1?"+params+"";
		},
		ajaxCount(){
			var that = this;
			var rs = null;
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/company_staff/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"staffName":staffName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/staff/company_staff/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
