				...
				<el-table
					...
					<el-table-column label="异常附图"  width="200px">
						...
						<el-table-column
							...
							width="240">
							<template v-slot="{ row }">
								<template v-if="row.ycWarning.dealStatus=='1'">
									<el-button
										href="javascript:void(0)" :data-params="'id='+row.ycWarning.id+''" data-href="/gzw/yc_warning/qryj_page" @click="goToPage"
								...
								<template v-if="row.ycWarning.dealStatus=='1'">
									<el-button
										href="javascript:void(0)" :data-params="'id='+row.ycWarning.id+''" data-href="/gzw/yc_warning/qxyj_page" @click="goToPage"
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/gzw/yc_warning/getInitData",
				async: false,
				data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/gzw/yc_warning/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"ycType":ycType,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/gzw/yc_warning/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
