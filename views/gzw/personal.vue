...
<template>
	<div class="detail-page">
		<div class="detail-page-div">
			<h2 class="detail-page-name">国资委</h2>
						...
						<div class="admin-ibox-content">
							<div   class="detail-page-div5">
								...
								<div class="detail-page-btn-area">
									<el-button  href="javascript:void(0)" :data-params="'id='+id+''" data-href="/gzw/gzw/update1_page" @click="goToPage" type="warning">修改</el-button>
								</div>
							</div>
						</div>
					</div>
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/gzw/gzw/getDetailData",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.id = result.detail.gzw.id;
					that.loginName = (result.detail.gzw.loginName==null?'':result.detail.gzw.loginName);
					that.name = (result.detail.gzw.name==null?'':result.detail.gzw.name);
					that.celphone = (result.detail.gzw.celphone==null?'':result.detail.gzw.celphone);
					that.remark = (result.detail.gzw.remark==null?'':result.detail.gzw.remark);
				}
			});
		},
	}
