	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/gzw/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="登录名">
						<template v-slot="{ row }">
							<div v-html="''+(row.gzw.loginName==null?'':row.gzw.loginName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="姓名">
						<template v-slot="{ row }">
							<div v-html="''+(row.gzw.name==null?'':row.gzw.name)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="联系电话">
						<template v-slot="{ row }">
							<div v-html="''+(row.gzw.celphone==null?'':row.gzw.celphone)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="备注">
						<template v-slot="{ row }">
							<div v-html="''+(row.gzw.remark==null?'':row.gzw.remark)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.gzw.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.gzw.id+''" data-href="/admin/gzw/update_page" @click="goToPage"
				...
				<div>
					<div id="gzw_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/gzw/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/gzw/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"loginName":loginName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/gzw/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
