			...
			<div >
				<el-table
					...
					<el-table-column
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.zbMsg.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.zbMsg.id+''" data-href="/admin/zb_msg/update_page" @click="goToPage"
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/zb_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/zb_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"cgid":that.cgid,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/zb_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
