...
<template>
	<div class="edit-page-div">
		<div class="box-card">
			<h2 class="edit-page-div-h2">中标信息-修改</h2>
...
export default {
	...
	mounted() {
		...
		setZoneImg(zbFileDrop,that.zbFile);
		setSelectableVal('pbzj',that.pbzj)
		date_utils.setDomVal("zbTime",that.zbTime);
	},
	methods:{
		getDataList(){
			var that = this;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/zb_msg/getInitData",
				async: false,
				data : {
		...
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/zb_msg/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
					'loginToken':that.currentLoginToken,
				...
				success : function(result) {
					that.zbs=result.zbs;
					that.lxr=result.lxr;
					that.lxfs=result.lxfs;
					that.zbFile=result.zbFile;
					that.pbzj=result.pbzj;
					that.zbTime=result.zbTime;
				}
			});
		},
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/admin/zb_msg/update_submit",
				async: false,
				data : {
					'id':that.id,
					'cgid':that.cgid,
