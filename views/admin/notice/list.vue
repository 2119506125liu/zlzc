	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/notice/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="标题">
						<template v-slot="{ row }">
							<div v-html="''+(row.notice.title==null?'':row.notice.title)+''"></div>
						</template>
					</el-table-column>
					<el-table-column label="封面图"  width="200px">
						...
						<el-table-column
							label="发布时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.notice.fbTime==null?'':row.notice.fbTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.notice.id+''" data-href="/admin/notice/detail" @click="goToPage"
									type="success" style="margin-left:5px;margin-top:5px;"
									>
									查看详情
								</el-button>
								<el-button
									href="javascript:void(0)" :data-id="row.notice.id" @click="del"
									type="danger" style="margin-left:5px;margin-top:5px;"
									>
									删除
								</el-button>
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.notice.id+''" data-href="/admin/notice/update_page" @click="goToPage"
					...
					<div>
						<div id="notice_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/notice/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/notice/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/notice/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
