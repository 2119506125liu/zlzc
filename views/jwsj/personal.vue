...
<template>
	<div class="detail-page">
		<div class="detail-page-div">
			<h2 class="detail-page-name">纪委书记</h2>
									...
									<div class="admin-table">
										<div id="detailBody">
											...
											<div class="admin-table-div">
												...
												<div class="col2" v-html="remark==null?'':remark"></div>
											</div>
										</div>
									</div>
								</div>
								<div class="detail-page-btn-area">
									<el-button  href="javascript:void(0)" :data-params="'id='+id+''" data-href="/jwsj/jw_book/update1_page" @click="goToPage" type="warning">修改</el-button>
								</div>
							</div>
						</div>
					</div>
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/jwsj/jw_book/getDetailData",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.id = result.detail.jwBook.id;
					that.loginName = (result.detail.jwBook.loginName==null?'':result.detail.jwBook.loginName);
					that.name = (result.detail.jwBook.name==null?'':result.detail.jwBook.name);
					that.celphone = (result.detail.jwBook.celphone==null?'':result.detail.jwBook.celphone);
					that.remark = (result.detail.jwBook.remark==null?'':result.detail.jwBook.remark);
				}
			});
		},
	}
