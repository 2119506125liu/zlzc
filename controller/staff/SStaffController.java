...
public class SStaffController{
	...
	*/
	...
	public Object getDetailData(HttpServletRequest request) {
		...
		Staff preModel = staffMapper.selectByPrimaryKey(login.getId());
		Map<String,Object> rs = new HashMap<String,Object>();
		rs.put("detail",staffService.getStaffModel(preModel,login));
		return rs;
	}
	/**
	修改提交信息接口
	*/
	...
	@ResponseBody
	public Object update1_submit(Staff model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = staffService.update1(model,login);//执行修改操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
