...
@CrossOrigin(origins = "*")
@Controller
@RequestMapping("/admin/yc_warning")
public class AYcWarningController{
	...
	public void getList(Map<String,Object> rs ,LoginModel login,YcWarning model){
		...
		JwBookExample jwBookE = new JwBookExample();
		List<JwBook> jwBookList=jwBookMapper.selectByExample(jwBookE);
		List<Map<String,Object>>  jwBookList2 = new ArrayList<Map<String,Object>>();
		for(JwBook m:jwBookList){
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("id",m.getId());
			map.put("name",m.getLoginName());
			jwBookList2.add(map);
		}
		rs.put("jwBookList",jwBookList2);
	}
	/**
	* 根据查询条件分页查询异常预警数据总数
	*/
	...
	*/
	...
	@ResponseBody
	public Object update_submit(YcWarning model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = ycWarningService.update(model,login);//执行修改操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
