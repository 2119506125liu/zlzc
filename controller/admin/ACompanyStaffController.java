...
@CrossOrigin(origins = "*")
@Controller
@RequestMapping("/admin/company_staff")
public class ACompanyStaffController{
	...
	*/
	...
	@ResponseBody
	public Object add_submit(CompanyStaff model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = companyStaffService.add(model,login);//执行添加操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
	...
	*/
	...
	@ResponseBody
	public Object update_submit(CompanyStaff model,ModelMap modelMap,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> rs = new HashMap<String,Object>();
		String msg = companyStaffService.update(model,login);//执行修改操作
		if(msg.equals("")==false){
			rs.put("code",0);
			rs.put("msg",msg);
			return rs;
	...
	*/
	@RequestMapping(value="export_company")
	public void export_company(CompanyStaff model,Integer page,HttpServletRequest request,HttpServletResponse response,ModelMap modelMap){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String,Object> data =  companyStaffService.getDataList(model,page,CommonVal.pageSize,login);//获取数据列表，不分页
		companyStaffService.exportCompany( request,  response,data, login);
	}
	/**
	* 生成批量导入导入excel模板
	*/
	@RequestMapping(value="import_company_model")
	public void import_company_model(HttpServletResponse response,HttpServletRequest request){
	...
	**/
	@RequestMapping(value = "import_company", method = RequestMethod.POST)
	@ResponseBody
	public Object importCompany(@RequestParam MultipartFile file,ModelMap modelMap,HttpServletRequest request,
	CompanyStaff model, HttpServletResponse response)throws IOException {
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);
		Map<String, Object> rs = new HashMap<String, Object>();
		String data = companyStaffService.importCompany(file.getInputStream(),file.getOriginalFilename(), login,model);// 导入数据
		if (!data.equals("")) {
			rs.put("code", 0);
			rs.put("msg", data);
			return rs;
